...
export class UserDataComponent implements OnInit {
  ...
  public readonly subscriptionDetailsURL =
    GITHUB_REPO + 'wiki/Subscription-Service#Background';
  ...
  async loadUserData() {
    this.userDataService
      .getUserData(this.linkDomain, this.pluginConfig.token)
      .subscribe({
        next: (data) => {
          this.userData = data;
      ...
      .then((response) => {
        if (response.success) {
          window.homebridge.toast.success(
            ...
            this.translateService.translations['toast.title_success'],
          );
  ...
  renderPayPalButtons(): void {
    if (
      ...
      !this.userData?.paypalPlans?.length
    ) {
      console.log(
        '❌ No PayPal plans available or subscription creation not expanded',
      );
      return;
    }
  ...
  async renderPayPalButton(plan: any, index: number): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      (window as any).paypal
        .Buttons({
          createSubscription: (data, actions) => {
            console.log(
              ...
              this.userData.startDate.toISOString(),
            );
        ...
        .catch((err) => {
          console.error(
            `❌ Failed to render PayPal button for plan ${plan.id}:`,
            err,
          );
